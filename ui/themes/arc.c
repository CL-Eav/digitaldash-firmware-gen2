...
static void event_cb(lv_event_t * e)
{
    ...
    {
    	int32_t target_value = scale_float(data->pid->pid_value, data->pid->precision);
...
lv_obj_t * add_arc_gauge( int32_t x, int32_t y, int32_t w, int32_t h, lv_obj_t * parent, GAUGE_DATA* data)
{
    ...
    lv_obj_t *circle = lv_obj_create(gauge);
    lv_obj_set_size(circle, RADIAL_SIZE, RADIAL_SIZE);
    lv_obj_align(circle, LV_ALIGN_TOP_MID, 0, y+5);

    lv_obj_set_style_radius(circle, RADIAL_SIZE/2, 0);    // make it a circle
    ...
    lv_obj_set_style_arc_color(needle, lv_color_hex(0x404040), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_arc_rounded(needle, true, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_arc_rounded(needle, true, LV_PART_INDICATOR | LV_STATE_DEFAULT);
